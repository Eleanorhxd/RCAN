...
class Transformer(nn.Module):
    ...
    def decode(self, memory, src_mask, tgt, tgt_mask, past=None, memory_matrix=None):
        ...
        # Memory querying and responding for textual features
        context_aware_bi_gate = ContextAwareBiGate(embeddings.size(-1), embeddings.size(-1)).to(embeddings.device)
        embeddings = context_aware_bi_gate(embeddings, responses)
...
class ContextAwareBiGate(nn.Module):
    ...
    def forward(self, input, context):
        forward_gate = torch.sigmoid(self.linear_forward(input)) #前向门
        backward_gate = torch.sigmoid(self.linear_backward(input)) # 后向门
...
class BaseCMN(AttModel):
    ...
    def _prepare_feature_forward(self, att_feats, att_masks=None, seq=None):
        ...
        # bigate = ContextAwareBiGate(att_feats.size(-1), att_feats.size(-1)).to(att_feats.device)
        # att_feats = bigate(att_feats, responses)
        # Memory querying and responding for visual features
