...
#     # Remove any keys in the checkpoint's state_dict that are not in the current model's state_dict
#         state_dict = {k: v for k, v in state_dict.items() if k in current_state_dict}
...
class Tester(BaseTester):
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            test_res.to_csv(os.path.join(self.save_dir, "res_mimic.csv"), index=False, header=False)
            test_gts.to_csv(os.path.join(self.save_dir, "gts_mimic.csv"), index=False, header=False)
        return log
        ...
        with torch.no_grad():
            for batch_idx, (images_id, images, reports_ids, reports_masks) in enumerate(self.test_dataloader):
                ...
                for ne_idx, ne in enumerate(ner(" ".join(report)).ents):
                    if ne.start_char >= len(char2word) or ne.end_char >= len(char2word):
                        continue  # Skip entities that have out-of-range indices
                    ...
                    for layer_idx in range(len(attention_weights[0])):
                        ...
                        if len(attn) == 0:
                            continue  # Skip cases where there are no attention weights for the entity
                            ...
                            cv2.imwrite(os.path.join(self.save_dir, "attentions_entities", "{:04d}".format(batch_idx),
                                 "layer_{}".format(layer_idx), "{:04d}_{}.png".format(ne_idx, ne)), heatmap)
