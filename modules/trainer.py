...
class BaseTrainer(object):
    ...
    def train(self):
        ...
        filename = os.path.join(self.record_dir, 'VRCA 0.9ce+0.1wcl.csv')
...
class Trainer(BaseTrainer):
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (images_id, images, reports_ids, reports_masks) in enumerate(self.train_dataloader):
            ...
            output, graph_loss = self.model(images, reports_ids, mode='train')
            # output = self.model(images, reports_ids, mode='train')

            loss = 0.9 * self.criterion(output, reports_ids, reports_masks) + 0.1 * graph_loss
            # loss = 0.9 * self.criterion(output, reports_ids, reports_masks)
        ...
        with torch.no_grad():
            ...
            for batch_idx, (images_id, images, reports_ids, reports_masks) in enumerate(self.test_dataloader):
                ...
                for i in range(len(reports)):
                    pre = reports[i]
                    gt = ground_truths[i]
                    df_list.append({'key': images_id[i], 'gt': gt, 'pred': pre})
            ...
            test_met_series = pd.Series(test_met)  # Convert the float to a Series
            df = pd.concat([df, test_met_series], ignore_index=True)  # Concatenate the Series to the DataFrame
            # df = pd.concat(test_met, ignore_index=True)
